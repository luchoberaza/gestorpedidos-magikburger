...
def _is_delivery_label(label: Any) -> bool:
    """True si el ajuste corresponde a un envío (lo que crea el botón 'Cobrar envío',
    label 'Envío'). Normaliza a minúsculas y sin acentos para tolerar variantes."""
    s = (str(label or "")).strip().lower()
    for a, b in (("á", "a"), ("é", "e"), ("í", "i"), ("ó", "o"), ("ú", "u"), ("ü", "u")):
        s = s.replace(a, b)
    return s.startswith("envio")
...
def generate_daily_orders_pdf_bytes(conn, include_adjustments=True):
    ...
    # ---------- cash summary ----------
    # Envíos por pedido (ajustes 'Envío', positivos). El envío es un extra que se queda
    # el repartidor: no se rinde, así que lo separamos del efectivo a rendir.
    delivery_by_order = {}
    if _table_has_column(conn, "order_adjustments", "order_id"):
        try:
            for _r in conn.execute("SELECT order_id, label, amount_cents FROM order_adjustments;").fetchall():
                _amt = int(_r["amount_cents"] or 0)
                if _amt > 0 and _is_delivery_label(_r["label"]):
                    _oid = int(_r["order_id"])
                    delivery_by_order[_oid] = delivery_by_order.get(_oid, 0) + _amt
        except sqlite3.Error:
            delivery_by_order = {}

    courier_seen = set()
    courier_cash_sales = {}     # solo ventas en efectivo (sin transfer), incluyen el envío
    courier_delivery_cash = {}  # solo los envíos cobrados en efectivo (se descuentan de lo que rinde)
    for o in orders:
        ...
        if not is_transfer(pm):
            ...
            courier_delivery_cash[cname] = courier_delivery_cash.get(cname, 0) + delivery_by_order.get(int(o["id"]), 0)
    ...
    story.append(Paragraph("Incluye el cambio configurado de cada repartidor si tuvo pedidos. "
                           "El envío queda para el repartidor: se descuenta de lo que rinde.", s_muted))
    story.append(Spacer(1, 6))
    ...
    if not courier_seen:
        ...
        rows = [[
            Paragraph("<b>Repartidor</b>", s_txt),
            Paragraph("<b>Efectivo</b>", s_txt),
            Paragraph("<b>Envíos</b>", s_txt),
            Paragraph("<b>A rendir</b>", s_txt),
        ]]
        for cname in sorted(courier_seen):
            handled = courier_cash_sales.get(cname, 0) + _courier_float_cents(conn, cname)
            delivery = courier_delivery_cash.get(cname, 0)
            to_render = handled - delivery
            rows.append([
                Paragraph(cname, s_txt),
                Paragraph(f"${money(handled)}", s_txt),
                Paragraph(f"${money(delivery)}", s_muted),
                Paragraph(f"<b>${money(to_render)}</b>", s_kpi),
            ])

        st = Table(rows, colWidths=[None, 30 * mm, 28 * mm, 32 * mm], hAlign="LEFT")
        st.setStyle(
            TableStyle(
                [
...
def api_liquidation():
    ...
    try:
        ...
        # --- Envíos por pedido (ajustes 'Envío', positivos) ---
        # El envío es un extra que se queda el repartidor: NO debe sumarse a lo que
        # rinde. Lo separamos para descontarlo del efectivo a rendir y mostrarlo aparte.
        delivery_by_order: Dict[int, int] = {}
        try:
            if _table_has_column(conn, "order_adjustments", "order_id"):
                for r in conn.execute("SELECT order_id, label, amount_cents FROM order_adjustments;").fetchall():
                    amt = int(r["amount_cents"] or 0)
                    if amt > 0 and _is_delivery_label(r["label"]):
                        oid = int(r["order_id"])
                        delivery_by_order[oid] = delivery_by_order.get(oid, 0) + amt
        except sqlite3.Error:
            delivery_by_order = {}
        ...
        total_cents = 0
        transfer_cents = 0
        delivery_total_cents = 0       # todos los envíos del día (lo que le queda al repartidor)
        delivery_cash_total_cents = 0  # solo envíos cobrados en efectivo (se descuentan del efectivo a rendir)

        if has_payment:
            rows = conn.execute("SELECT id, total_cents, payment_method FROM orders;").fetchall()
            for r in rows:
                total_cents += int(r["total_cents"] or 0)
                dz = delivery_by_order.get(int(r["id"]), 0)
                delivery_total_cents += dz
                ...
                if pm.startswith("transf") or "transfer" in pm or "banco" in pm or "mp" in pm or "mercado" in pm:
                    ...
                    delivery_cash_total_cents += dz
        else:
            rows = conn.execute("SELECT id, total_cents FROM orders;").fetchall()
            for r in rows:
                total_cents += int(r["total_cents"] or 0)
                dz = delivery_by_order.get(int(r["id"]), 0)
                delivery_total_cents += dz
                delivery_cash_total_cents += dz  # sin payment_method, todo se asume efectivo
        ...
        if has_payment:
            orders = conn.execute(f"""
                SELECT c.id AS courier_id, c.name AS courier_name, {float_sel}
                       o.id AS order_id, o.total_cents, o.payment_method
                FROM couriers c
        ...
        for o in orders:
            ...
            if cid not in data:
                ...
                data[cid] = {
                    ...
                    "delivery_cents": 0,       # envíos del repartidor (lo que se queda)
                    "delivery_cash_cents": 0,  # envíos cobrados en efectivo (se descuentan del efectivo)
                }

            data[cid]["orders_count"] += 1
            data[cid]["total_cents"] += int(o["total_cents"] or 0)

            dz = delivery_by_order.get(int(o["order_id"]), 0)
            data[cid]["delivery_cents"] += dz
            ...
            if has_payment:
                ...
                if pm.startswith("transf") or "transfer" in pm or "banco" in pm or "mp" in pm or "mercado" in pm:
                    ...
                    data[cid]["delivery_cash_cents"] += dz
            else:
                data[cid]["delivery_cash_cents"] += dz
        ...
        for d in data.values():
            ...
            # − sus envíos cobrados en efectivo (el envío es del repartidor, no se rinde).
            d["cash_cents"] = d["total_cents"] - d["transfer_cents"] - d["delivery_cash_cents"]
        ...
        # − los envíos cobrados en efectivo (quedan para el repartidor).
        cash_to_render_cents = cash_base_cents + cash_float_total_cents - delivery_cash_total_cents
        ...
        return jsonify({
            ...
            "delivery_total_cents": delivery_total_cents,
            "delivery_cash_total_cents": delivery_cash_total_cents,
            ...
            "delivery_total": money(delivery_total_cents),
            "couriers": [
                {
                    **d,
                    ...
                    "delivery": money(d.get("delivery_cents", 0)),
                }
                for d in data.values()
            ]
        })
